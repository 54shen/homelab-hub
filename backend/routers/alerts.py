...
def create_alert(req: AlertRuleCreate, db: Session = Depends(get_db), token=Depends(auth_write)):
...
def update_alert(rule_id: int, req: AlertRuleUpdate, db: Session = Depends(get_db), token=Depends(auth_write)):
...
def toggle_alert(rule_id: int, req: AlertRuleToggle, db: Session = Depends(get_db), token=Depends(auth_write)):
    rule = db.query(AlertRule).filter(AlertRule.id == rule_id).first()
    if not rule:
        raise HTTPException(404, "规则不存在")
...
def delete_alert(rule_id: int, db: Session = Depends(get_db), token=Depends(auth_write)):
    rule = db.query(AlertRule).filter(AlertRule.id == rule_id).first()
    if rule:
        db.delete(rule)
        db.commit()
